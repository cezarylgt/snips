...
def _format_to_valid_collection(s: str):
...
def _clear_dates(*objects) -> List[dict]:
    def _rm(o):
        del o['created_at']
        del o['updated_at']
        return o

    return [_rm({**o}) for o in objects]
...
class TestCli:
    ...
    @pytest.fixture(autouse=True)
    # @mock.patch.dict(os.environ, {"DB_URI": "/home/cezaryl/test-db.json"})
    def _setup(self):
        # set eniron
        os.environ['DB_URI'] = '/home/cezaryl/test-db.json'
        result = runner.invoke(app, ['config', 'set', 'format', 'json'])
        assert result.exit_code == 0
    ...
    def test_cli_show_by_alias(self):
        print(runner.invoke(app, ['config', 'show']).stdout)
        result = runner.invoke(app, ['show', 'test1'])
        assert result.exit_code == 0
        response = json.loads(result.stdout)
        expected = self.snippet1.dict()
        assert _clear_dates(response) == _clear_dates(expected)

    def test_cli_ls_should_print_all_snippets(self):
        result = runner.invoke(app, ['ls'])
        assert result.exit_code == 0
        response = json.loads(_format_to_valid_collection(result.stdout))
        expected = [self.snippet1.dict(), self.snippet2.dict()]
        assert _clear_dates(*response) == _clear_dates(*expected)
    ...
    def test_cli_ls_with_tags_should_print_snippets_only_containing_given_tags(self):
        ...
        assert result.exit_code == 0
        response = json.loads(_format_to_valid_collection(result.stdout))
        assert _clear_dates(*response) == _clear_dates(self.snippet2.dict())
